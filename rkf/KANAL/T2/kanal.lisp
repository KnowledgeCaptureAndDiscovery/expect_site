  ...
  (let ((proposed-actions 
    ...
    (dolist (action-and-vals proposed-actions)
      ...
      (cond ((null failed-step) ;; case of unachieved expected effect
	     (kanal-format-verbose stream " proposed fix : add a ~S step with ~S as ~S to change the ~S of ~S~%"
		     p-action description-of-vals val role object)
	     (generate-fix-item  (list 'add p-action '{with description-of-vals 'as val '} 'to-make role 'of object 'become val) 
				 ':add-step)
	     )
	    (t     ;; case of unachieved precondition
	     (kanal-format-verbose stream " proposed fix : add a ~S step with ~S as ~S before step ~S to change ~S of ~S~%"
		     p-action description-of-vals val failed-step role object)
	     (generate-fix-item (list 'add p-action '{with description-of-vals 'as val '} 'before failed-step 'to-make role 'of object 'become val) 
