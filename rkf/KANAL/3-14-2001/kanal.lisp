...
NOTE: in the content form, the upper-cased names are variables.
    ...
    :add-step - add an action when there is an unachieved effect
       content-form - (add ACTION to-make ROLE of OBJECT become VAL)

    :add-step-before-step - add an action for unachieved precondition
       content-form (add ACTION before FAILED-STEP to-make ROLE of OBJECT become VALUE)

    :modify-step - modify a step
       content-form - (modify STEP to-make ROLE of OBJECT become VALUE)
              ;; in case of unachieved effect

            - (modify STEP before FAILED-STEP to-make ROLE of OBJECT become VALUE)
    ...
    :delete-link
       content-form - (delete-unnecessary-links in loops LOOPS-FOUND)
       ;; example: (delete-unnecessary-links in loops ((S1 S2 S1) (S2 S3 S4 S2)))

    :add-expr
       content-form - (add EXPR as ROLE 'of OBJECT)
       ;; example: (fix add (|the| |Exit| |subevents| |of|
       ;;                      (|the| |Release| |subevents| |of|
       ;;                       (|the| |Deliver| |subevents| |of| |_TestVirusInvadesCell6|)))
       ;;                     as |next-events| of |_Degrade14|)
...
;;; File: kanal.lisp
;;; Author: Jihie Kim
;;; Date:  Wed Mar 14 15:05:21 PST 2001
...
(defun simulate-steps-and-find-failed-events (model-instance start-situation)
  ...
  (simulate-events (find-first-primitive-events 
		    model-instance) start-situation)
  *failed-event-info*
  )
...
(defun init-test-process-model (process-model)
  ...
  (let* ((start-situation (create-situation))
	 (model-instance (create-instance process-model start-situation)))
      (add-ordering-info model-instance) ;; for RNA transcription
...
(defun build-action-table ()
  ...
  (dolist (action (get-all-actions))
	...
	(if (null (get-role-values action-instance role))
      ...
      (setq effects (union (get-the-add-list action-instance)
...
(defun compute-add-del-lists-1 (action) ;; concept
  (let ((action-instance (create-instance action))
    ...
    (dolist (role primary-roles)
      (if (null (get-role-values action-instance role))
	  (assert-role-value action-instance role (create-a-thing))))
    (union (get-the-add-list action-instance)
	   (get-the-del-list action-instance))
      ))
  ...
  (let ((proposed-actions 
    ...
    (dolist (p-action proposed-actions)
      (cond ((null failed-step) ;; case of unachieved expected effect
	     (kanal-format-verbose stream " proposed fix : add a ~S step  to change the ~S of ~S~%"
		     p-action role object)
	     (generate-fix-item  (list 'add p-action 'to-make role 'of object 'become val) 
				 ':add-step)
	     )
	    (t     ;; case of unachieved precondition
	     (kanal-format-verbose stream " proposed fix : add a ~S step before ~S to change ~S of ~S~%"
		     p-action failed-step role object)
	     (generate-fix-item (list 'add p-action 'before failed-step 'to-make role 'of object 'become val) 
...
(defun action-can-change-role-value-of-object (action object role)
  (let* ((concepts (get-concepts-from-instance object))
    ...
    (can-change-role-value-of-object add-del-lists concepts object role)
    ))
...
(defun can-change-role-value-of-object (add-del-lists concepts object role)
  (let ((result nil))
    (dolist (affect add-del-lists)
...
;; changed for RNA transcription scenario
...
(defun find-actions-that-can-change-role-value-1 (object role concept)
  (let ((actions (get-subconcepts concept)) 
    ...
    (dolist (action actions)
      (if (action-can-change-role-value-of-object action object role)
	  (setq result (append result (list action)))
...
(defun suggest-to-modify-steps-that-can-achieve-the-effect (object role val failed-step
  ...
  (let ((affected-actions (find-current-steps-that-can-change-role-value 
    ...
    (dolist (p-action affected-actions)
      (cond ((null failed-step) ;; case of unachieved expected effect
	     (kanal-format-verbose stream " proposed fix : modify ~S step  to change ~S of ~S~%"
		     p-action role object)
	     (generate-fix-item (list 'modify p-action 'to-make role 'of object 'become val) ':modify-step))
	    (t     ;; case of unachieved precondition
	     (kanal-format-verbose stream " proposed fix : modify a ~S step before ~S to change ~S of ~S~%"
		     p-action failed-step role object)
	     (generate-fix-item (list 'modify p-action 'before failed-step 'to-make role 'of object 'become val) 
				':modify-step) ))
	)
  ...
  (let ((affected-actions (find-current-steps-that-undid-effect 
    ...
    (cond ((or (null affected-actions) (null (second affected-actions)))
	  ...
	  (t 
	   ...
	   (generate-fix-item (list 'change-or-add-link-between-steps affected-actions) 
...
(defun find-current-steps-that-can-change-role-value (object role steps-before)
  (let ((result nil))
    (dolist (step steps-before)
      (if (step-can-change-role-value-of-object step object role)
	  (setq result (append result (list step)))
	))
    result
    )
...
(defun find-fixes-for-loop (expr source)
  (generate-fix-item (list 'delete-unnecessary-links 'in 'loops expr) ':delete-link)
  )
...
(defun test-knowledge (concept &key (cwa t))
  (init-test)
...
(defun get-proposed-fixes (error-id)
  (kanal-format *kanal-log-stream* "~%~% ===============================================")
  (kanal-format *kanal-log-stream* "~% Getting fixes for ~S~%" error-id)
  (setq *fix-items* nil)
  (setq *error-report-silent* t)
