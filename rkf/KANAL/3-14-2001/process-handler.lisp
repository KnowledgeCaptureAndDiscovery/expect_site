...
(defun test-preconditions (preconditions situation &optional (event nil))
  (let ((failed-preconditions nil)
    ...
    (if event (kanal-format *kanal-log-stream* "~&   preconditions:~S ~%~%" preconditions))
      ...
      (cond ((and event (invalid-triple triple :precondition event))
	    ...
	    (t
		 ...
		 (cond ((and (null *test-with-closed-world-assumption*)
			     event (triple-consistent-with-kb triple situation))
			...
			(let ((object (second triple))
			  ...
			  (kanal-format *kanal-log-stream* 
					"~% ~S ~%  ~S ~%     is not explicitly true but consistent with the KB" 
...
(defun make-event-situation-list (events situation disjunctive?)
  (mapcar #'(lambda (event) (list event situation disjunctive?)) events)
  )
...
(defun simulate-events-1 (event-situation-list &optional (stream *kanal-log-stream*))
  (let* ((doable-events (find-doable-events event-situation-list))
	 (undoable-events (set-difference event-situation-list doable-events :test #'equal))
	 (doable-event (first doable-events))
	 (curr-situation nil)
	 (simulation-result nil)
	 (next-events nil)
	 )
    (kanal-format *kanal-log-stream* "~% simulate-events-1 ~S~%" event-situation-list)
    ...
    (cond ((null doable-event)
	  ...
	  (t
	   (setq simulation-result 
		 (simulate-step (first doable-event) (second doable-event) (third doable-event)))))
	   ...
	   (unless (third doable-event) ;; non disjunctive loop
	     (setq next-events
			...
			(make-event-situation-list
			 ...
			 (second doable-event) nil) next-events))
	     ))
	   ...
	   (setq next-events
		 (append (make-event-situation-list
			  (filter-out-top-event
			   (find-next-disjunctive-events (first doable-event)))
			  curr-situation t) next-events))
	   (setq next-events 
		 (append (make-event-situation-list
			  (filter-out-top-event
			   (find-next-events (first doable-event)))
			  curr-situation nil) next-events))
	   )
	  )
    ...
    (unless (equal simulation-result 'fail) 
      (if undoable-events
	  (kanal-format stream "~%   [Events ~a were undoable this iteration...]~%" 
		     (mapcar #'first undoable-events)))
      (let* ((next-events-to-simulate 
	      (remove-duplicates
	       (append next-events (rest doable-events)
		       undoable-events) :test #'equal :from-end t)))
  ...
  (let* ((preconditions (get-preconditions event))
    ...
    (cond (failed-preconditions
	  ...
	  (t
	   ...
	   (find-causal-link preconditions curr-situation event)
...
(defun report-causal-links (&optional (stream *kanal-log-stream*))
  (when *causal-links*
    ...
    (mapcar #'(lambda (causal-link)
		(kanal-format stream "~% ~S enabled ~S by achieving~%    ~S"
			(first causal-link)
			(second causal-link)
			(third causal-link)))
	    *causal-links* )
    ...
    (cond ((and (null first-subevents) ;; primitive event itself
	  ...
	  ((null first-subevents);; this shouldn't happen
	   (let ((first-subevents-desc (get-role-description event 
	     ...
	     (cond ((null first-subevents-desc)
		    ;(kanal-format stream "~% WARNING (missing ordering constraints)~%")
		    ;(kanal-format stream "  ordering between  ~S isn't defined~%" subevents)
		    ;(kanal-format stream "  the system do not execute these subevents~%")
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; These functiosn are used when all the events are executed
...
(defun print-event-info (event-info-struc &optional (stream *kanal-log-stream*)
  ...
  (kanal-format stream "~%[ Event Info ~% path: ~S ~% prev-situation: ~S ~% next-situation ~S ~% added effects(accumulated):   ~S ~% deleted effects(accumulated): ~S~%]"
...
(defun report-simulated-paths (&optional (stream *kanal-log-stream*))
  (cond ((null *paths-simulated*)
	 (generate-error-checker-item (get-all-executable-events)
				      ':unreached-events
				      ':warning nil
				      nil)
	 nil)
	(t 
      ...
      (mapcar #'(lambda (path)
		  (let* ((events (mapcar #'get-event path))
			 (unreached-events
			  (set-difference 
			   (get-all-executable-events) events)))
		    ...
		    (when unreached-events
		      (generate-error-checker-item unreached-events
						   ...
						   ':warning nil
						   events)
		      (kanal-format stream "~% ~S ~% were not reached in this path" unreached-events)
		      )
...
(defun cancelled-equivalent-effect-in-del-list (effect prev-deleted-effects &optional (stream *kanal-log-stream*))
  (let ((new-del-list nil)
	...
	(cond ((or (equal effect del-effect)
	       ...
	       (kanal-format stream "~%      canceling effects: adding ~S ~%         which has been deleted by earlier action ~S"
...
(defun cancelled-equivalent-effect-in-add-list (effect prev-added-effects
  ...
  (let ((new-add-list nil)
	...
	(cond ((or (equal effect add-effect)
	       ...
	       (setq found-equivalent-effect t)
	       (kanal-format stream "~%      canceling effects: deleting ~S ~%         which has been added by earlier action ~S"
