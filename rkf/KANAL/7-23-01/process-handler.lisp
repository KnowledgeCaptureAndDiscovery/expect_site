...
(defun get-first-subevents (event)
  (let ((result (get-role-values-with-checking "first-subevent" event nil)))
...
(defun check-if-not-parallel (events)
  (dolist (e1 events)
	...
	(kanal-format nil "~%there is an ordering between parellel events (~S before ~S) ~%" e1 e2)
...
(defun get-all-subevents (event)
  (let ((look-up-result (assoc event *all-subconcepts-used*)))  ;; for efficiency
    (cond ((null look-up-result)
	   (let ((result (get-role-values-with-checking "all-subevents" event nil)))
...
(defun get-all-subevents (event)
  (let ((look-up-result (assoc event *all-subconcepts-used*)))  ;; for efficiency
    (cond ((null look-up-result)
	   (let ((result (remove-duplicates (get-all-subevents-1 event))))
	     (setq *all-subconcepts-used*
		   (acons event result *all-subconcepts-used*))
	     result))
	  (t ;(format t "~% lookup:event found ~S~%" event)
...
(defun get-all-subevents-1 (event)
  (cond ((null event)
	 nil)
	((let* ((first-subevents (get-first-subevents event))
	   ...
	   (cond (first-subevents-that-are-not-subevents
		  (mapcar #'(lambda (e)
			      (generate-error-checker-item 'ordering
						       :missing-subevent
						       :error nil
						       (list event e)))
			  first-subevents-that-are-not-subevents)
		  (append subevents first-subevents-that-are-not-subevents
			  (mapcan #'get-all-subevents-1 (append subevents first-subevents-that-are-not-subevents)))
		  )
		 (t (append subevents
			    (mapcan #'get-all-subevents-1 subevents)))))))
  )
  ...
  (let* ((effects (filter-out-null-effects add-list del-list))
    ...
    (cond ((and (null added-effects)
	   ...
	   (generate-error-checker-item nil
					...
					event))
	  (t
	   (generate-error-checker-item (list added-effects deleted-effects)
					':no-effect
					':warning
					t
					event))
  ...
  (let ((prev-situations (previous-situation situation)))
	...
	(cond ((null failed-preconditions) ;; not failed
		 ...
		 (if (not (member new-causal-link *causal-links* :test #'equal))
	       ...
	       (kanal-format nil "~%~% ***** ~S enabled ~S by achieving or asserting (for inexplicit preconditions)~%    ~S~%"
  ...
  (let ((prev-situations (previous-situation situation)))
	...
	nil
      (let* ((prev-situation-info (first prev-situations))
	     (prev-event (third prev-situation-info))
	     (prev-situation (second prev-situation-info))
	     (add-list (get-the-add-list prev-event))
	     (del-list (get-the-del-list prev-event))
	...
	(cond (satisfied-conditions
		 ...
		 (if (not (member new-causal-link *causal-links* :test #'equal))
		     (setq *causal-links* (append *causal-links*
						  (list new-causal-link)))))
	       (kanal-format nil "~%~% ***** ~S enabled ~S by achieving or asserting ~%    ~S~%"
...
(defun report-simulated-paths (&optional (stream *kanal-log-stream*))
  (cond ((null *paths-simulated*)
         (kanal-format stream "~% There was no simulation path~%~%")
