...
(setf *top-level-goal*
      '(evaluate (obj (inst-of plan)))
      )

(setf *ps-known*
      '(
        ;;(evaluation-aspect (inst-of plan))
        (checked-resource (inst-of resource-check))
        ))

;;; Adding a concept to this list means any instances appearing in the
;;; ps-tree will cause relations used to become necessary for the class.
(push 'resource 
      expect::*concepts-whose-instances-should-be-generalized-for-links*)

;;; Change some defaults in the method analyzer because of the way these
;;; methods will most likely be used.
(setf expect::*analyzer-show-unused-plans* nil)
(setf expect::*analyzer-show-unused-variables* nil)

;;; Ultimately I want to have the PSM library methods be distinguished
;;; by some role filled in at the plan, but for now I just list the
;;; names of all the library methods, so modify-plan menus and such-like
;;; can ignore them.
(setf expect::*psm-library-method-names*
      '(
        set-up-evaluation-aspects
        EVALUATE-PLAN EVALUATE-GENERAL-ASPECT EVALUATE-ALL-RESOURCE-USAGE-IN-A-PLAN
        EVALUATE-A-RESOURCE-IN-A-PLAN EVALUATE-A-GLOBAL-RESOURCE-GLOBALLY
        EVALUATE-A-LOCAL-RESOURCE-LOCALLY EVALUATE-CONSUMABLE-CAPACITATED-RESOURCE
        EVALUATE-REUSABLE-CAPACITATED-RESOURCE ESTIMATE-LINEAR-RESOURCE
        EVALUATE-REUSABLE-CAPACITATED-RESOURCE-ON-PLAN-TASK
        SUM-SET-OF-PLAN-TASK-USEAGES DEFAULT-FIND-PLAN-TASKS-SHARING-RESOURCE
        DEFAULT-FIND-PLAN-TASKS-SHARING-TEMPORAL-SPATIAL-RESOURCE
        CHECK-PLAN-TASKS-SHARE-TEMPORAL-SPATIAL-RESOURCE
        DEFAULT-ESTIMATE-CAPACITATED-RESOURCE-AVAILABLE-TO-PLAN-TASK
        EVALUATE-REUSABLE-DISCRETE-RESOURCE
        EVALUATE-REUSABLE-DISCRETE-RESOURCE-ON-PLAN-TASK
        RELATION-CHECK-REUSABLE-DISCRETE-RESOURCE-NEEDED
        RELATION-CHECK-REUSABLE-DISCRETE-RESOURCE-AVAILABLE
        RELATION-ESTIMATE-CAPACITATED-RESOURCE-FOR-PLAN-TASK CHECK-NO-AMOUNT-NEEDED
        RELATION-ESTIMATE-AVAILABILITY-OF-CAPACITATED-RESOURCE CHECK-CONSTRAINTS
        EVALUATE-LOCAL-CONSTRAINT EVALUATE-LOCAL-BOUNDS-CHECK
        EVALUATE-LOCAL-UPPER-BOUND EVALUATE-LOCAL-LOWER-BOUND
        DEFAULT-ESTIMATE-UPPER-BOUND DEFAULT-ESTIMATE-LOWER-BOUND
        ESTIMATE-NUMBER-OF-PARENTS RELATION-EVALUATE-LOCAL-CONSTRAINT-ON-PLAN-TASK
        evaluate-global-completeness-critique evaluate-global-constraint
        CHECK-CHILDREN CHECK-PARENTS SET-MEMBER CLASS-SET-MEMBER COUNT-THE-ELEMENTS
        CHECK-EQUALITY FIND-THE-INSTANCES-OF-A-CONCEPT IS-IT-AN-INSTANCE-OF-GENERAL
        DETERMINE-WHETHER-THERE-ARE-ANY-GENERAL
        combine-groups-of-things
        blurgh blurgh2		; must find better names..
        ))

;;; Actions and roles
(def-expect-action 'combine)
...
(def-expect-action 'set-up-evaluation-aspects)
(def-expect-action 'find-fillers)
...
(defrelation plan-tasks-sharing :is-primitive expect-action-role)
;;; This one is not yet used here, but is in some specific domains.
(defrelation shares :is-primitive expect-action-role)
  ...
  '(
    ...
    ((name evaluate-plan)
     ...
     (method (evaluate (obj ?p)
		   (with-respect-to
		    (evaluation-aspect 
		     (set-up-evaluation-aspects (obj ?p))))
		   )))
...
|#
    ...
    ((name evaluate-reusable-capacitated-resource)
     ...
     (method (determine-whether
	    ...
	    (of (evaluate (obj 
		         ;; This is COA specific. Sorry.
		         (critical-event-or-coa-task ?p)
		         )
    ...
    ((name estimate-linear-resource)
     ...
     ((name blurgh)
      ...
      (method (find (obj (spec-of expect::maximum))
		(of (estimate (obj amount) (of (checked-resource ?r)) 
			    (needed-by ?s)))))
      ;; post-processing creates separate critique objects for each task
     ...
     ((name blurgh2)
      ...
      (post-processing
       (store-availability-estimate-for-task ?method-result ?o ?r)))
...
|#
     ...
     ((name evaluate-global-completeness-critique)
      ...
      (post-processing (progn 
		     (add-to-evaluation ?method-result ?c ?p)
		     (add-grandchild-result-as-analysis-estimate ?c))))
