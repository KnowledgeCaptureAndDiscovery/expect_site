...
(defun produce-xml-method-tree (item node-name get-subs print-item-name
  ...
  (format nil "~%~VT<gr-node>~A~{~A~}~%~VT</gr-node>" 
          (* 3 level)
          (produce-xml-generic-element node-name 
                                       (list (list "name" (funcall print-item-name item))
     				     (list "package" (find-package-name
						      (funcall print-item-name item)))
				     (list "id" (funcall print-item-id item))
				     (list "desc" (funcall print-item-desc item))
				     (list "body" (funcall print-body item))
				     (list "body-desc" (funcall print-body-desc item))
				     (list "edt" (funcall print-edt item))
				     (list "result" (funcall print-result item))
				     (list "simplified-capability" (funcall print-goal item))
				     (list "capability" (funcall print-capability item))
				     (list "system-proposed-name" (funcall print-system-proposed-name item))
				     (list "highlight-result" (funcall print-highlight-result item))
				     ;; FOR PSM Tool PSTree UI:  uses a method that has just been defined
				     (list "using-new-method" (node-uses-new-method-p-1
				     			       expect-tree-node))
				     ;;whether the node's children should be shown in the UI
				     (list "expand" (expand-node-in-ui-p
				     		     expect-tree-node))
				     
				     ;(list "possible-users" (funcall print-possible-users item))
				     )
...
(defun get-goal-desc-from-undefined-goal (undefined-goal)
  ;;(produce-method-xml (second (create-goal-desc-from-simplified-goal (get-undefined-goal-goal undefined-goal))) 'method-goal))
  (create-goal-desc-from-simplified-goal (get-undefined-goal-goal undefined-goal)))
...
(defun get-undefined-goal-list ()
  (produce-xml-undefined-goal-list "undefined-goals" #'get-undefined-goal-goal
			     ...
			     #'get-undefined-goal-expected-result #'get-goal-desc-from-undefined-goal
...
(defun delete-all-plans (&key context)
  ...
  (format *terminal-io* "~%*plan-kb-current* : ~%~S" *plan-kb-current*)
  )
...
(defun get-simplified-goal (method)
  (let* ((*package* (find-package "EXPECT"))
    ...
    (format *terminal-io* "::method-name ~S" method-name)
    (if (not (null plan))
	(format nil "~%~A~%"
...
(defun xml-dismiss-agenda-item (item-name-in-string)
  (cond ((invalid-lisp-object item-name-in-string)
	...
	(t
	 (let ((agenda-item (find-agenda-item-with-item-name item-name)))
	   (if (null agenda-item)
