...
(defvar *system-created-goals* nil)  ;; may not need this
...
(defun find-subgoals-and-compute-edt (body cap name expected-result)
  (if *mkb-very-verbose*
     (if *problem-solver-verbose* (format *terminal-io* "~% START find-subgoals-and-edt ~%  body:~S ~%  cap:~S ~%"
             body cap)))
  (let ((temp
  (cond
   ((null body) '())
   ...
   ((lexicon-relation-p (car body))
    (let ((param-list (cdr body)) (simplified-params nil))
      (dolist (param param-list)
	;;(if *problem-solver-verbose* (format *terminal-io* "~% param:~S" param))
	(setq simplified-params
	      (append simplified-params
		      (list (find-subgoals-and-compute-edt param cap name 'undefined)))))
      ;(if *problem-solver-verbose* (format *terminal-io* "~% rel : ~S ~%  simplified params ~S" (car body) simplified-params))
   ...
   (if *mkb-very-verbose*
      (if *problem-solver-verbose* (format *terminal-io* "~% find-subgoals-and-edt ~%  body:~S ~%  cap:~S ~% --> ~S"
	      body cap temp)))
  temp)
  )
...
(defun find-result-type-of-goal (goal plan-name expected-result)
  (if (null plan-name)
      (find-result-type-of-goal-without-adding-plan goal expected-result)
    (find-result-type-of-goal-1 goal plan-name expected-result)))

(defun find-result-type-of-goal-1 (goal plan-name expected-result)
  (setq *partial-match* nil)
  ;(if *problem-solver-verbose* (format *terminal-io* "********** reset partial-match 1 "))
  ;(if *problem-solver-verbose* (format *terminal-io* "~% ### find-result-type-of-goal ~S for ~S" goal plan-name))
  (cond (;;(member 'undefined (mapcar #'second (rest goal)))
	...
	(t ;; args are all defined
	 ...
	 (if *mkb-verbose*
	     (if *problem-solver-verbose* (format *terminal-io* "~% *rfmls-unmatched* reset in find-result-type-of-goal")))

	 (let ((subsumers (relaxed-find-parents goal)))
	   (cond ((null subsumers)
		  ;;(if *problem-solver-verbose* (format *terminal-io* "~%  create undefined ~S" goal))
		  (create-undefined-goal goal plan-name
					 expected-result *rfmls-unmatched*)
		  (if *mkb-very-verbose*
		      (if *problem-solver-verbose* (format *terminal-io* "~% ### *mkb-undefined-goal-list*: ~S"
			      *mkb-undefined-goal-list*)))
		  ;(if *problem-solver-verbose* (format *terminal-io* "~% add system goal -- ~S" goal))
		 ...
		 (t
		  (let ((result-edt 'undefined))
		    (dolist (subsumer subsumers)
		      (if *mkb-verbose*
			  (if *problem-solver-verbose* (format *terminal-io* "~% ** for subsumer ~S : relaxed? ~S "
			...
			(when (not (null mkb-method)) ;; when not a system defined goal
			  (cond (*partial-match*
				...
				(t
				 ...
				 (if (null plan-name) (format t "~% ERRROR: push nil in users in find-result-type-of-goal~%"))
    ...
    (let* ((rgoal (first rgoals))
      ...
      (cond ((null subsumers)
	     ...
	     (setq *rfmls-unmatched* ;
		   (pushnew (list (first rgoals) 'covering) *rfmls-unmatched* :test #'equal))
	     other-parents)
...
(defun find-parents-from-rfml (sgoal)
  (if *mkb-very-verbose*
      (if *problem-solver-verbose* (format *terminal-io* "~% rfml for GOAL: ~S" sgoal)))
  (let* ((sgoals (mapcar #'caadr 
    ...
    (if *mkb-very-verbose*
        (if *problem-solver-verbose* (format *terminal-io* "~% set rfml for ~S ==>  ~S" sgoals parents-from-set-rfml)))
...
(defun find-parent-to-add-goal (sgoal)
  (find-parent-to-add-goal-1 sgoal (list *mkb-capability-tree-root*) nil)
  )
  
(defun find-parent-to-match (sgoal)
  (let ((goal-node
	 (find-parent-to-add-goal-1 sgoal (list *mkb-capability-tree-root*) nil)))
    (if goal-node (find-defined-subsumer goal-node))
    ))

(defun find-parent-to-add-goal-1 (sgoal cands parent)
  (let ((cand (first cands)))
  (cond ((null cands) parent)
	((equal (first cands) *mkb-capability-tree-root*)
	 (find-parent-to-add-goal-1 sgoal (get-goal-node-children cand) cand))
	(t
	 (let ((subsumption-test-result
		(goal-subsumed--create-system-goal sgoal
			    (get-goal-node-goal cand))))
	   (cond ((equal 't subsumption-test-result) ;; matched with existing node
		  (if *mkb-very-verbose*
		     (if *problem-solver-verbose* (format *terminal-io* "~% ~S is subsumed by ~S~%"
			     sgoal (get-goal-node-goal cand))))
		  (find-parent-to-add-goal-1 sgoal (get-goal-node-children cand) cand))
		 ((null subsumption-test-result) ;; no match, no new goal created
		  (find-parent-to-add-goal-1 sgoal (rest cands) parent))
		 (t ;; try to create a new goal node
		  (let ((new-goal-node
			 (try-to-create-goal-node subsumption-test-result nil nil)))
		    (cond ((null new-goal-node) ;; already exists
			   (find-parent-to-add-goal-1 sgoal (rest cands) parent))
			  (t
			   ;(if *problem-solver-verbose* (format *terminal-io* "~% newly created system node ~S" subsumption-test-result))
			   (find-parent-to-add-goal-1 sgoal (list new-goal-node) nil)))))
	   )))
)))
...
(defun find-parent-1 (sgoal cands parent)
  (let ((cand (first cands)))
    (cond ((null cands) parent)
	((equal (first cands) *mkb-capability-tree-root*)
	 (find-parent-1 sgoal (get-goal-node-children cand) cand))
	((goal-subsumed sgoal (get-goal-node-goal cand)) ;; matched
	 (find-parent-1 sgoal (get-goal-node-children cand) cand))
	(t ;; not matched 
	  (find-parent-1 sgoal (rest cands) parent)))))
...
(defun goal-subsumed--create-system-goal (sub super)
  (if *mkb-very-verbose*
      (if *problem-solver-verbose* (format *terminal-io* "~% goal subsumed?: ~S ~S" sub super)))
  (let ((super-actions (get-all-super-goal-concepts (first sub))))
    (cond ((and (null (rest super)) ;; system defined action-name
		(or (member (first super) super-actions)
		    (eq (first sub) (first super))))
...
(defun common-edt-of-args (args-a args-b)
  ;(if *problem-solver-verbose* (format *terminal-io* "~% common-edt-of-args : ~S ~S" args-a args-b))
...
(defun create-subsumer-as-system-goal (sub super)
  ...
  (let ((subsumer (append (list
    ...
    (when (and (not (equal sub subsumer))
      ...
      (if *mkb-verbose*
	(if *problem-solver-verbose* (format *terminal-io* "~% create-subsumer-as-system-goal : ~S" subsumer))
	)
...
(defun args-compatible (args-sub args-super)
  (cond ((and (null args-sub) (null args-super))
	...
	((not (eq (caar args-sub) (caar args-super))) ; check param name
	 nil)
	((not (or (sub-edt-p (cadar args-sub)
			     (cadar args-super))
		  (edt-compatible (remove-set-of (cadar args-sub))
...
(defun args-subsumed (args-sub args-super)
  (cond ((and (null args-sub) (null args-super))
	 (if *mkb-very-verbose* (if *problem-solver-verbose* (format *terminal-io* "~% both null args")))
	 t)
	((or (null args-sub) (null args-super))
	 (if *mkb-very-verbose*
	     (if *problem-solver-verbose* (format *terminal-io* "~% one of them null args")))
	 nil)
	((and (eq (caar args-sub) (caar args-super))
	      (equal (mkb-find-common-edt (cadar args-sub)
					  (cadar args-super))
		     (remove-inst-or-desc (cadar args-super))))
	 (if *mkb-very-verbose*
	     (if *problem-solver-verbose* (format *terminal-io* "~% subsumed param  ~S ~S"
	...
	(t
	 (if *mkb-very-verbose*
	     (if *problem-solver-verbose* (format *terminal-io* "~% not subsumed param  ~S ~S"
		 (cadar args-sub) (cadar args-super))))
	 nil)))
...
(defun add-goal-in-sub-tree (goal parent children)
  (if *mkb-verbose*
      (if *problem-solver-verbose* (format *terminal-io* "~% add-goal-in-subtree:~S ~% children: ~S"
  ...
  (let ((subsumee nil))
	...
	(push child subsumee)))
    (if *mkb-verbose*
	(if *problem-solver-verbose* (format *terminal-io* "~% subsumee:~S" (mapcar #'get-goal-node-goal subsumee))))
...
(defun try-to-create-goal-node (simplified-goal mkb-method defined-p)
  (let ((parent (find-parent simplified-goal)))
    (if (equal simplified-goal (get-goal-node-goal parent))
	nil
      (add-goal-node simplified-goal mkb-method defined-p parent))))

(defun add-goal-node (simplified-goal mkb-method defined-p &optional (given-parent nil))
  (let ((parent (if given-parent
		    given-parent
		  (find-parent-to-add-goal simplified-goal))))
    ;(if *mkb-verbose*
        (if *problem-solver-verbose* (format *terminal-io* "~%parent of ~S is ~%   == ~S"
	      simplified-goal (get-goal-node-goal parent)))
     ; )
    (cond ((equal simplified-goal (get-goal-node-goal parent))
	   ;(if *problem-solver-verbose* (format *terminal-io* "~% same goal exists"))
	  ...
	  (t (let ((goal-node (make-goal-node
			       ...
			       :desc (if mkb-method
					 (get-plan-capability-nl
					  (get-mkb-method-plan mkb-method))
				       (generate-nl-desc-of-expr simplified-goal))
		   ...
		   (dolist (c super-goal-concepts)
		     (add-goal-node (list c) nil nil (find-parent (list c)) )))
	       goal-node
...
(defun find-edt-for-var (var assoc-list)
  (if *mkb-very-verbose*
      (if *problem-solver-verbose* (format *terminal-io* "~% ~S in ~S~%" var assoc-list)))
...
(defun revise-undefined-goal-list (mkb-method)
  (let ((plan-names
	...
	(let ((edt (compute-plan-result (get-mkb-method-plan method))))
	  (if *mkb-verbose*
	      (if *problem-solver-verbose* (format *terminal-io* "~% new subgoals for:~S  is --- ~S~%"
  ...
  (let ((goal (get-mkb-method-goal mkb-method))
    ...
    (dolist (ug *mkb-undefined-goal-list*)
      ...
      (cond ((or (goal-subsumed (get-undefined-goal-goal ug) goal) ;;*****
		...
		;			(list name))
		;		       (mkb-method-possible-users mkb-method))))
	     (if *mkb-verbose* (if *problem-solver-verbose* (format *terminal-io* "~% ug added again ~S" ug)))
	     (push ug undefined-goals)))))
    (setq *mkb-undefined-goal-list* undefined-goals)
    ;(if *problem-solver-verbose* (format *terminal-io* "~% NEW UNDEFINED GOALS ~% ~S" undefined-goals))
    result))
...
(defun add-system-defined-goals ()
  ...
  (dolist (sg *system-created-goals*)
    (if *mkb-verbose*
	(if *problem-solver-verbose* (format *terminal-io* "~% $$  add-system-defined-goal ~S" sg))
      )
	   ...
	   (let* ((subsumers (find-parents-from-rfml goal))  ;; recompute rfml
	     ...
	     (cond ((null subsumers) ;;; check if the new method participated???
		   ...
		   (*partial-match* ;; not fully subsumed
		    ...
		    nil)
		   (t
		    ;;(if *problem-solver-verbose* (format *terminal-io* "~%rfml-subsumed ~S ~S" goal new-goal))
		    't))
