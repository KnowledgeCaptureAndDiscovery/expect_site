	...
	System.out.println("In actionPerformed of CapText");
	  if (state == SELECTING_OLD) 
	  {
		  ...
		  if(ResultString.length() > 0)
		  {
		      ...
		      //the second element of the String is being accessed bcos
		      //the first character is not recognizable
		      Integer ResultInteger = new Integer(ResultString.substring(1));
		  ...
		  //instantiate a frame for the user to choose entries
	          Frame ListOptions = new ListChoices(ListPresent);
		  ...
		  for(int i=0; i< ListPresent.size(); i++)
		      System.out.println("Choice #"+i+" : " + ListPresent.get(i).toString());
