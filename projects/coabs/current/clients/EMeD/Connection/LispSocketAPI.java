...
public class LispSocketAPI {
  ...
  // Allow creating without running the socket dialog, to make startup easier.
  public LispSocketAPI(String passedDefaultHostname, String passedDefaultPort,
		   boolean runDialog) {
    defaultPort = passedDefaultPort;
    port = Integer.parseInt(defaultPort);
    hostname = passedDefaultHostname;
    if (runDialog == true) {
      runConnectionDialog(passedDefaultHostname, defaultPort);
    }
    try {
      openServerConnection(hostname, port);
    } catch (FactoryException e2) {
    ...
    public void sendLispCommand(String command) {
        ...
        System.out.println("In sendLispCommand:" + command);
        if (serverSocket != null)
	  outStream.println(command + "eOl");
        //outStream.println("(:eval " + command + ")");
        //System.err.println("Printing: " + command + "eOl");
    }
    ...
    public String readLispResult() throws FactoryException {
   	...
   	System.out.println("In readLispResult");

	try {
          System.out.println("In try of readLispResult");
    ...
    //****************************************
    // Like evaluateLispCommand, but assumes that a predicate returning
    // either NIL or a non-NIL value is interpreted as a boolean.
    //****************************************
    public boolean evaluateLispPredicate (String predicate) throws FactoryException {
        return evaluateLispPredicate(predicate, "CL-USER");
    }

    public boolean evaluateLispPredicate (String predicate, String lispPackage) {
	String result = evaluateLispCommand(predicate, lispPackage);
	System.out.println("Evaluation returns " + result + " which means " + result.equalsIgnoreCase("NIL"));
	return !result.equalsIgnoreCase("NIL");
    }
    ...
    public Vector readList() {
        ...
        // that.
        return readList(safeReadLine(), this);
    }
    ...
    public static Vector readList(String line) {
        return readList(line, (LispSocketAPI)null);
    }

    public static Vector readList(String line, LispSocketAPI lc) {
	  ...
	  if (Character.isSpaceChar(line.charAt(j))) {
		...
		readingElement = false;
		if (j > k) {
		    // replace "NIL" by an empty vector.
		    if (line.substring(k,j).equals("NIL") == true) {
		...
		readingElement = false;
		readingString = false;
		if (line.substring(k+1,j).equals("NIL") == true) {
		    res.addElement(new Vector());
		} else {
	  ...
	  j++;
	  // This could cause the process to hang if the line was just
	  // malformed, but I need to be able to read lists with
	  // strings that contain newlines. I can only do this when
	  // there is an instance of LispSocketAPI around, though,
	  // since this method is static.
	  if (j == line.length() && 
	      (readingString == true || readingSublist == true)
	      && lc != null) {
	      line = line + '\n' + lc.safeReadLine();
	  }
        }
	  ...
	  // Even if we were reading a sublist, add as a string
	  // because the line ended prematurely.
	  if (line.substring(k).equals("NIL") == true) {
	      res.addElement(new Vector());
	  } else {
