...
public class MEditor extends JPanel
  ...
  private JButton commitButton = new JButton("select");
  ...
  public MEditor (String editType, ExpectSocketAPI theServer, 
	        JFrame theFrame, 
    ...
    initRes = resultDesc;
    methodName = name;
    System.out.println("in editor resultDesc:"+resultDesc);
    ...
    if (editType.equals("CopyAndCreate")||editType.equals("Modify")) {
      String desc = es.getNLOfMethod(name);
      if (null== desc)  sketchArea = new JTextArea("To do what (Capability):\nDo (Method Body):\n\nThe result will be (Result Type): "+initRes);
      else {
	int i = desc.indexOf("To do that");
	if (i>0) {
	  String description = desc.substring(0,i-1) + "\n(Body):" + desc.substring(i);
	  desc = description;
	}
	sketchArea = new JTextArea("(Capability):"+desc+"\n(Result Type):The result will be "+es.getNLOfMethodResult(name));
      }
    }
    else 
  ...
  private void initMethodPanel (String initCapability) {
    ...
    if (initCapability == null){
      ...
      String nlCap = es.getNLOfCapability(initCapability);
      String nlResult = es.getNLOfDataType(initRes);
      sketchArea.setText("To do what (Capability): "+nlCap+"\nDo (Method Body):\nThe result will be (Result Type): "+nlResult);
       int i;
      JPanel p= new JPanel();
      p.setLayout(new FlowLayout(FlowLayout.LEFT));
      String xmlString = es.getEditCapOrBody(initCapability,"nil");
      System.out.println(" cap:"+initCapability);
  ...
  // get saved capabilities from previous method search results
  public Vector getSearchedCapabilities () {
    String xmlString;
    String methodName;
    methodDescRenderer mr;
    if (thePanel == null) return null;
    Vector methodNames = thePanel.getSearchedMethods();
    Vector results = new Vector();
    for (int i =0; i<methodNames.size(); i++) {
      methodName = (String) methodNames.elementAt(i);
      xmlString = es.getSearchedCapability(methodName);
      //System.out.println("methodName:"+methodName);
      // System.out.println("xml:"+xmlString);
      mr = new methodDescRenderer(xmlString);
      results.addElement(mr.getCapButtons());
    }
    return results;
  }
  ...
  public void layoutContainer(Container c) {
    ...
    undoButton.setBounds (b.width-85, inset*3+550, 80, 30);
    commitButton.setBounds (b.width-85, inset*3+450, 80, 30);
    //quitButton.setBounds (inset+410, inset*4+610, 100, 30);
  }

  public Vector getSystemGoalsButtons () {
    return thePanel.getSystemGoalsButtons();
  }
