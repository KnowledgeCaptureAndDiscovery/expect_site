...
public class ExpectWindowPanel extends JPanel  {
  ...
  JList listBox;
  JToggleButton toggle,newtoggle;
  String etype;  
  ...
  public ExpectWindowPanel(String type) {
    ...
    englishPanel = new Tree.methodListPanelEnglish(es,this,etype);
    
    System.out.println("added method list panel");
    maker = createMakerFrame(englishPanel, "Method List");
    //maker.setBounds (2,2, 550, 700);  // paper mode
    //maker.setBounds (2,2, 460, 670); // demo mode
    maker.setBounds (2,2, 600, 830); // on Sun
    JComponent c = (JComponent) maker.getContentPane();
    //c.setLayout(new BorderLayout());
    toggle = new JToggleButton("Formal");
    c.add(toggle, BorderLayout.NORTH);
  ...
  class toggleListener implements ActionListener {
  	...
  	public void actionPerformed(ActionEvent e)
  	{
  		//toggle.setText("English");
        
      reloadFormal();
    }  
      /*  listPanel = new Tree.methodListPanel(es,parent,"normal");
        newmaker = createMakerFrame(listPanel, "Method List");
        newmaker.setBounds (2,2, 600, 830);
        lc.removeAll(); 
        lc.add(newmaker, JLayeredPane.PALETTE_LAYER);  
        //lc.updateUI();
        JComponent c = (JComponent) maker.getContentPane();
        c.setLayout(new BorderLayout());
    	toggle = new JToggleButton("English");
    	c.add(toggle, BorderLayout.NORTH);
        if (etype.equals("small")) {
        sTabbedPanel = new SmallTabbedPanel (es, parent);
        maker = createMakerFrame(sTabbedPanel, "Views");
        }
      else {
        eTabbedPanel = new ExpectTabbedPanel(es,parent);
        newmaker = createMakerFrame(eTabbedPanel, "Views");
      }
    //maker.setBounds (455,2, 500, 580);
      newmaker.setBounds (605,2, 600, 830); //sun
  ...
  class newtoggleListener implements ActionListener {
...
private void reloadFormal () {
    System.out.println ("Reload All");
    lc.removeAll(); 
    //listPanel = new Tree.methodListPanelEnglish(es,this,etype);
    listPanel = new Tree.methodListPanel(es,this,etype);
    newmaker = createMakerFrame(listPanel, "Method List");
    //maker.setBounds (2,2, 450, 580);
    newmaker.setBounds (2,2, 600, 830); // sun
    //maker.setBounds (5,10, 550, 700);
    //maker.setBounds (2,2, 460, 670); // demo mode
    //lc.add(newmaker, JLayeredPane.PALETTE_LAYER);  
    JComponent c = (JComponent) newmaker.getContentPane();
    //c.setLayout(new BorderLayout());
    newtoggle = new JToggleButton("English");
    c.add(newtoggle, BorderLayout.NORTH);
    //c.add(new JButton(), BorderLayout.CENTER);
    newtoggle.addActionListener(new newtoggleListener()); 
    lc.add(newmaker, JLayeredPane.PALETTE_LAYER);
    if (etype.equals("small")) {
      sTabbedPanel = new SmallTabbedPanel (es, this);
      newmaker = createMakerFrame(sTabbedPanel, "Views");
    }
    else {
      eTabbedPanel = new ExpectTabbedPanel(es,this);
      newmaker = createMakerFrame(eTabbedPanel, "Views");
  ...
  private void reloadEnglish () {
    System.out.println ("Reload All");
    lc.removeAll(); 
    //listPanel = new Tree.methodListPanelEnglish(es,this,etype);
    englishPanel = new Tree.methodListPanelEnglish(es,this,etype);
    newmaker = createMakerFrame(englishPanel, "Method List");
    //maker.setBounds (2,2, 450, 580);
    newmaker.setBounds (2,2, 600, 830); // sun
    //maker.setBounds (5,10, 550, 700);
    //maker.setBounds (2,2, 460, 670); // demo mode
    ...
    JComponent c = (JComponent) newmaker.getContentPane();
    //c.setLayout(new BorderLayout());
    newtoggle = new JToggleButton("Formal");
    c.add(newtoggle, BorderLayout.NORTH);
    //c.add(new JButton(), BorderLayout.CENTER);
    newtoggle.addActionListener(new toggleListener()); 
    lc.add(newmaker, JLayeredPane.PALETTE_LAYER);
    if (etype.equals("small")) {
      sTabbedPanel = new SmallTabbedPanel (es, this);
      newmaker = createMakerFrame(sTabbedPanel, "Views");
