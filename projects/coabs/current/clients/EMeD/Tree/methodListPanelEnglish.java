...
public class methodListPanelEnglish extends JPanel {
  Dimension   origin = new Dimension(0, 0); 

  private expandableTreeModel treeModel;
  private expandableTree tree,systree;
  ...
  public methodListPanelEnglish(ExpectSocketAPI theServer, 
		     ExpectWindowPanel parent,
		     String itype) {
    thePanel = parent;
    interfaceType = itype;
    expandableTreeNode top = new expandableTreeNode("Methods");
    ...
    tree.setCellRenderer(new engListCellRenderer());
    es = theServer;
    ...
    String xmlInput = es.getNLUserDefinedMethodNameList("");
    //String xmlInput = es.getNLOfAllMethods();
    
    System.out.println("user defined methods"+xmlInput);
    englishListRenderer er = new englishListRenderer(xmlInput);
    
    root = er.getMessagesAsTreeNode();
    //commented latest
    ...
    expectMessages = er.getMessagesAsString();*/
    //System.exit(0);
    //userMethodsRoot = thisRenderer.getRootOfMethodTree(xmlInput);
    treeModel.insertNodeInto(root, top, 0);

    methodListRenderer thisRenderer1 = 
      new methodListRenderer(theServer,"System Defined Methods");
    String xmlInput1= es.getNLSystemDefinedMethodNameList("");
    System.out.println("xml(system defined methods):"+xmlInput1);
    engSysListRenderer esr = new engSysListRenderer(xmlInput1);
    
    systemMethodsRoot = esr.getMessagesAsTreeNode();
    /*treeModel = new expandableTreeModel(systemMethodsRoot);
    tree = new expandableTree(treeModel);*/
    //System.exit(0);
    /*systemMethodsRoot = thisRenderer1.getRootOfMethodTree(xmlInput1);*/
    treeModel.insertNodeInto(systemMethodsRoot, top, 1);
  ...
  private void modifyInNL() {      
    ...
    System.out.println("id"+currentName);  
    currentNode = lastItem;
    theParent = (expandableTreeNode)lastItem.getParent();
    theIndex = theParent.getIndex(lastItem) +1;
    theOperation = "modify";
