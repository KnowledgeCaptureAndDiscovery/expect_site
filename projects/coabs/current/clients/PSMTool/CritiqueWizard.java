...
public class CritiqueWizard extends JPanel {
  ...
  public NameFieldPanel nameField = null;
  
  public String critiqueName = null;
  
  private WizardPanel questionPanel = null,
    methodPanel = null, runPanel = null, currentPanel = null;
  ...
  public CritiqueWizard(LispSocketAPI passed_lc, //KATool passed_parent,
		    CritiqueView passed_cview,
		    NameFieldPanel passed_nameField,
		    String passedCritiqueName) {
    lc = passed_lc;
    //parent = passed_parent;
    critiqueView = passed_cview;
    nameField = passed_nameField;
    critiqueName = passedCritiqueName;
    if (critiqueName == null)
      critiqueName = "start";
    ...
    /* The panels are now set by the script, from the server.
       questionPanel = new WizardPanel("Part 1. Answer some questions about the critique");
    ...
    processReply("start", critiqueName);
  }
  ...
  public void processReply(String variable, String value) {
    String line;
    currentPanel = sectionPanel[0];
    int oldStep = step;
    Cursor oldCursor = getCursor();
    setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));

    //block commented by mukta
    // If the value has a space, send it to lisp in string quotes.
    /*if (value.indexOf(" ") == -1) {
      lc.sendLispCommand("(evaluation::wizard-respond-to-value '" + variable + " '" + value + ")");
    } else {
    ...
    if (value.indexOf(" ") == -1) {
      lc.sendLispCommand("(expect::wizard-respond-to-value '" + variable + " '" + value + ")");
    } else {
      // otherwise send it as a symbol or number.
      lc.sendLispCommand("(expect::wizard-respond-to-value '" + variable + " \"" + value + "\")");
    }
    ...
    while ((line = lc.safeReadLine()).equals("done") == false) {
      ...
      if (line.startsWith(":read ") == true) {
        addLine(new WizardReader(step++, line.substring(6), this));
      } else if (line.startsWith(":menu ") == true) {
        addLine(new WizardMenu(step++, line.substring(6), this));
	...
	public void actionPerformed(ActionEvent e) {
	  refreshPSTree();
        ...
        addLine(new WizardMethodLine(step++, line.substring(8), this));
      } else if (line.startsWith(":step ") == true) {
        addLine(new WizardStep(step++, line.substring(6), this));
    ...
    lc.safeReadLine();
    // Now recompute all the method summaries to see if any changed.
    for (int i = 0; i < step; i++)
      if (scriptLines[i] != null)
        scriptLines[i].updateSummary();
    // Once all the new lines are created from this round with the
    // server, we see if the server is prepared to answer any of the
    // questions it has asked. This can happen if we are displaying
    // what the script would do for an existing critique.
    for ( ; oldStep < step; oldStep++)
      scriptLines[oldStep].askServer();
    setCursor(oldCursor);
  }

  public void addLine(WizardLine line) {
    int afterIndex = -1;
    if (line.comesAfter != null && line.comesAfter.equals("NIL") == false) {
      // Look for the line whose variable matches this string
      boolean found = false;
      System.out.println("Looking for: " + line.comesAfter);
      for (int i = 0; i < step && found == false; i++) {
        if (scriptLines[i] != null)
	System.out.println(i + ": " + scriptLines[i].variable);
        if (scriptLines[i] != null && 
	  line.comesAfter.equals(scriptLines[i].variable) == true) {
	found = true;
	System.out.println("Found at " + i);
	// Now find that component's index in the panel. At the moment
	// fail if the requested line isn't in the current
	// panel. Later allow across-panel search..
	boolean foundPanel = false;
	for (int j = 0; 
	     foundPanel == false && j < currentPanel.getComponentCount(); 
	     j++) {
	  if (currentPanel.getComponent(j) == scriptLines[i]) {
	    foundPanel = true;
	    afterIndex = j + 1;
	  }
	}
    ...
    currentPanel.add(line, afterIndex);
    Dimension pref = currentPanel.getPreferredSize();
    currentPanel.setMaximumSize(new Dimension(9999, pref.height));
    currentPanel.setMinimumSize(new Dimension(pref.width, 0));
    currentPanel.revalidate();
